...
#define FINAL "Recreación_de_onda_sinusoidal.csv" 
...
int main(void)
{
	...
	FILE *fp_final;                                                             // Archivo final
	int cont = 0;                                                               // Contador utilizado para la impresión de caracteres
	...
	while(cont < 1000)
	{
		...
		printf("Valor de la conversión: %d, valor de contador: %d\n", ADCvalue, cont);
		fflush(stdout);
		cont++;
		usleep(1000);
	}

	fp_final = fopen(FINAL, "w");                                               // Abrir archivo final para escritura

	for(int i = 0; i < (MAX_LETRAS); i++)
	{
		fputs(Buffer[i], fp_final);	                                    // Escribir el contenido de StringArray hacia archivo fp_final
	}

	fclose(fp_final);	                                                        // Cerrar archivo
